...
class BankBookReport(models.TransientModel):
    ...
    def view_report(self):
        ...
        accounts = account_move_lines.mapped('account_id').read(
            ['display_name', 'name'])
        all_lines_data = account_move_lines.read(
        ...
        for line in all_lines_data:
            if line.get('account_id'):
                lines_by_acc[line['account_id'][0]].append(line)
        currency_id = self.env.company.currency_id.symbol
        for account in accounts:
            acc_name = account['display_name']
            move_line_data = lines_by_acc.get(account['id'], [])
            data[acc_name] = move_line_data
            move_lines_total[acc_name] = {
                'total_debit': round(sum(l['debit'] for l in move_line_data), 2),
                'total_credit': round(sum(l['credit'] for l in move_line_data), 2),
