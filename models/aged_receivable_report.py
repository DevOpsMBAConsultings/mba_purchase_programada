...
class AgeReceivableReport(models.TransientModel):
    ...
    def view_report(self):
        ...
        all_lines_data = paid.read(
            ['name', 'move_name', 'date', 'amount_currency', 'account_id',
             'date_maturity', 'currency_id', 'debit', 'move_id', 'partner_id'])
        lines_by_partner = defaultdict(list)
        for val in all_lines_data:
            difference = 0
            ...
            val['diff0'] = round(val['debit'], 2) if difference <= 0 else 0.0
            val['diff1'] = round(val['debit'], 2) if 0 < difference <= 30 else 0.0
            val['diff2'] = round(val['debit'], 2) if 30 < difference <= 60 else 0.0
            val['diff3'] = round(val['debit'], 2) if 60 < difference <= 90 else 0.0
            val['diff4'] = round(val['debit'], 2) if 90 < difference <= 120 else 0.0
            val['diff5'] = round(val['debit'], 2) if difference > 120 else 0.0
            ...
            if val.get('partner_id'):
                lines_by_partner[val['partner_id'][0]].append(val)
        for partner_id in partner_ids:
            move_line_data = lines_by_partner.get(partner_id.id, [])
            ...
            partner_total[partner_id.name] = {
                ...
                'diff4_sum': diff4_sum,
                'diff5_sum': diff5_sum,
                'debit_sum_display': format_number(debit_sum),
                'diff0_sum_display': format_number(diff0_sum),
                'diff1_sum_display': format_number(diff1_sum),
                'diff2_sum_display': format_number(diff2_sum),
                'diff3_sum_display': format_number(diff3_sum),
                'diff4_sum_display': format_number(diff4_sum),
                'diff5_sum_display': format_number(diff5_sum),
    ...
    def get_filter_values(self, date, partner):
        ...
        for val in all_lines_data:
            ...
            val['diff0'] = round(val['debit'], 2) if diffrence <= 0 else 0.0
            val['diff1'] = round(val['debit'], 2) if 0 < diffrence <= 30 else 0.0
            val['diff2'] = round(val['debit'], 2) if 30 < diffrence <= 60 else 0.0
            val['diff3'] = round(val['debit'], 2) if 60 < diffrence <= 90 else 0.0
            val['diff4'] = round(val['debit'], 2) if 90 < diffrence <= 120 else 0.0
            val['diff5'] = round(val['debit'], 2) if diffrence > 120 else 0.0
            if val.get('partner_id'):
                lines_by_partner[val['partner_id'][0]].append(val)
        for partner_id in partner_ids:
            move_line_data = lines_by_partner.get(partner_id.id, [])
