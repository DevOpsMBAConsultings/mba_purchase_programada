    ...
    def view_report(self):
        ...
        all_lines_data = paid.read(
            ['name', 'move_name', 'date', 'amount_currency', 'account_id',
             'date_maturity', 'currency_id', 'credit', 'move_id', 'partner_id'])
        lines_by_partner = defaultdict(list)
        for val in all_lines_data:
            diffrence = 0
            if val['date_maturity']:
                diffrence = (today - val['date_maturity']).days
            val['diff0'] = round(val['credit'], 2) if diffrence <= 0 else 0.0
            val['diff1'] = round(val['credit'], 2) if 0 < diffrence <= 30 else 0.0
            val['diff2'] = round(val['credit'], 2) if 30 < diffrence <= 60 else 0.0
            val['diff3'] = round(val['credit'], 2) if 60 < diffrence <= 90 else 0.0
            val['diff4'] = round(val['credit'], 2) if 90 < diffrence <= 120 else 0.0
            val['diff5'] = round(val['credit'], 2) if diffrence > 120 else 0.0
        ...
        for partner_id in partner_ids:
            ...
            partner_total[partner_id.name] = {
                'credit_sum': round(sum(val['credit'] for val in move_line_data), 2),
