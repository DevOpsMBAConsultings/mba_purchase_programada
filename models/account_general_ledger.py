...
class AccountGeneralLedger(models.TransientModel):
    ...
    def view_report(self, option, tag):
        ...
        for account in account_ids:
            move_line_list = lines_by_acc.get(account.id, [])
            # Group by move_id to produce list-of-lists (template uses valuelist[0])
            move_groups = defaultdict(list)
            for line in move_line_list:
                move_id = line.get('move_id') and line['move_id'][0]
                move_groups[move_id].append(line)
            account_dict[account.display_name] = list(move_groups.values())
            total_debit = round(sum(x['debit'] for x in move_line_list), 2)
            total_credit = round(sum(x['credit'] for x in move_line_list), 2)
