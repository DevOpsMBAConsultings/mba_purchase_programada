...
class AccountTrialBalance(models.TransientModel):
    ...
    def view_report(self):
        ...
        journals = self.env['account.journal'].search_read([], ['name'])
        ...
        init_by_acc = {x['account_id'][0]: (round(x['debit'], 2), round(x['credit'], 2)) for x in init_groups if x.get('account_id')}
        ...
        period_by_acc = {x['account_id'][0]: (round(x['debit'], 2), round(x['credit'], 2)) for x in period_groups if x.get('account_id')}

        move_line_list = []
        for account_id in account_ids:
            init_debit, init_credit = init_by_acc.get(account_id.id, (0.0, 0.0))
            init_diff = init_debit - init_credit
            if init_diff > 0:
                initial_total_debit = init_diff
                initial_total_credit = 0.0
            else:
                initial_total_debit = 0.0
                initial_total_credit = abs(init_diff)

            total_debit, total_credit = period_by_acc.get(account_id.id, (0.0, 0.0))

            if initial_total_debit == 0.0 and initial_total_credit == 0.0 and total_debit == 0.0 and total_credit == 0.0:
                continue
