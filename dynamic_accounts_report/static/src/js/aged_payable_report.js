...
class AgedPayable extends owl.Component {
    async setup() {
        ...
        this.state = useState({
            ...
            currency: null,
            total_credit: null,
            diff0_sum_display: null,
            diff1_sum_display: null,
            diff2_sum_display: null,
            diff3_sum_display: null,
            diff4_sum_display: null,
            diff5_sum_display: null,
    ...
    async load_data() {
        ...
        try {
            ...
            self.state.total_credit_display = (TotalCredit || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            self.state.diff0_sum = diff0Sum;
            self.state.diff0_sum_display = (diff0Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            self.state.diff1_sum = diff1Sum;
            self.state.diff1_sum_display = (diff1Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            self.state.diff2_sum = diff2Sum;
            self.state.diff2_sum_display = (diff2Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            self.state.diff3_sum = diff3Sum;
            self.state.diff3_sum_display = (diff3Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            self.state.diff4_sum = diff4Sum;
            self.state.diff4_sum_display = (diff4Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            self.state.diff5_sum = diff5Sum;
            self.state.diff5_sum_display = (diff5Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
        }
        catch (el) {
    ...
    gotoJournalEntry(ev) {
        ...
        return this.action.doAction({
            ...
            res_id: parseInt(ev.currentTarget.dataset.id, 10),
            views: [[false, "form"]],
            target: "current",
        });
    }
    gotoJournalItem(ev) {
        ...
        return this.action.doAction({
            ...
            name: "Journal Items",
            views: [[false, "list"]],
            domain: [["partner_id", "=", parseInt(ev.currentTarget.dataset.id, 10)], ['account_type', 'in', ['liability_payable']]],
            target: "current",
        });
    }
    openPartner(ev) {
    ...
    async applyFilter(ev, e, is_delete = false) {
        ...
        this.state.total_credit = TotalCredit;
            this.state.total_credit_display = (TotalCredit || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            this.state.diff0_sum = diff0Sum;
            this.state.diff0_sum_display = (diff0Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            this.state.diff1_sum = diff1Sum;
            this.state.diff1_sum_display = (diff1Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            this.state.diff2_sum = diff2Sum;
            this.state.diff2_sum_display = (diff2Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            this.state.diff3_sum = diff3Sum;
            this.state.diff3_sum_display = (diff3Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            this.state.diff4_sum = diff4Sum;
            this.state.diff4_sum_display = (diff4Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            this.state.diff5_sum = diff5Sum;
            this.state.diff5_sum_display = (diff5Sum || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    }
    getDomain() {
        return [];
    }
