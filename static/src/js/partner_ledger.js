...
class PartnerLedger extends owl.Component {
    ...
    async togglePartner(ev) {
        ev.preventDefault();
        ev.stopPropagation();
        
        let target = ev.currentTarget;
        let partner_id_str = target.getAttribute("data-id") || target.getAttribute("data-partner-id") || target.getAttribute("data-value");
        if (!partner_id_str) {
            console.error("No partner ID found on element:", target);
            return;
        }
        
        // Find the actual partner ID from the data or total
        let partner_name = partner_id_str;
        let actual_partner_id = null;
        if (this.state.total && this.state.total[partner_name]) {
            actual_partner_id = this.state.total[partner_name].partner_id;
        }
        
        if (!actual_partner_id) {
            console.error("Could not determine partner ID for:", partner_name);
            return;
        }
        
        // If lines are already loaded, just toggle visibility
        if (this.state.partner_lines[partner_name]) {
            // Lines exist, the XML template t-if will handle collapsing if we set it to null or we can just leave it
            // Actually, setting to null unmounts them, which is perfect for collapsing.
            delete this.state.partner_lines[partner_name];
            // Trigger reactivity
            this.state.partner_lines = { ...this.state.partner_lines };
            return;
        }
        ...
        try {
            ...
            this.state.partner_lines[partner_name] = lines || [];
            // Trigger reactivity
            this.state.partner_lines = { ...this.state.partner_lines };
            
        } catch (error) {
            console.error("Error fetching partner lines:", error);
            this.call('crash_manager', 'rpc_error', error);
        } finally {
            // Unblock UI
            unblockUI; // wait, unblockUI is not defined locally? Odoo 16+ OWL doesn't use unblockUI like this
        }
    }
