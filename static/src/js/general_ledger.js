...
// Added to color negative values red in Cybrosys Reports
(function() {
    let timeout;
    const observer = new MutationObserver((mutations) => {
        clearTimeout(timeout);
        timeout = setTimeout(() => {
            const targetElements = document.querySelectorAll('.table_main_view span, .table_main_view th, .table_main_view td, .table_main_view div');
            targetElements.forEach(el => {
                const text = el.textContent.trim();
                if (text.includes('.-') || text.includes('. -') || text.match(/^[-]\d/) || text.match(/^[a-zA-Z\$\/]+\s*\.-/)) {
