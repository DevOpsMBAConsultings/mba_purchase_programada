...
def _strip_reference_from_description(name, default_code):
    """Quita prefijo [referencia] de la descripción; el código ya va en CodigoProd, evita redundancia."""
    s = (name or "").strip()
    if not s or not default_code:
        return s or ""
...
class DigifactNUCBuilder(models.AbstractModel):
    ...
    def build_nuc_xml(self, move, allow_draft=False):
        ...
        # Effective fiscal header values: use move's values when set; in draft without reservation use placeholders for preview
        # IMPORTANT: Always use reserved fiscal number if available, even in draft mode
        # Check if fiscal number is reserved (all three fields must be present and non-empty)
        has_reserved_fiscal = bool(
            move.digifact_numero_df and 
            move.digifact_codigo_seguridad and 
            move.digifact_pto_fact_df
        )
        
        if has_reserved_fiscal:
            # Use reserved fiscal number (even in draft)
            effective_numero_df = move.digifact_numero_df
            effective_codigo_seg = move.digifact_codigo_seguridad
            effective_pto_fact = move.digifact_pto_fact_df
        elif move.state == "draft" and allow_draft:
            # Draft without reservation: use placeholders for preview only
            effective_numero_df = "0000000001"
            effective_codigo_seg = "000000001"
            pto_raw = (
                getattr(company, "x_fe_ptofact_code", None)
            ...
            # Posted invoice: must have fiscal number
            effective_numero_df = move.digifact_numero_df or "0000000001"
            effective_codigo_seg = move.digifact_codigo_seguridad or "000000001"
            effective_pto_fact = (
                move.digifact_pto_fact_df 
        ...
        dv_emisor = _get_company_fe(company, "dgi_dv", "x_fe_dgi_dv", "digifact_dv", default="")
        ...
        # If we have linked DGI locations on the move/partner, use them. Else fallback to standard Odoo fields.
        val_city = ""
        val_district = ""
        val_state = ""

        if move.dgi_partner_corregimiento_id:
            val_city = move.dgi_partner_corregimiento_id.name
        else:
            val_city = partner.city or ""

        if move.dgi_partner_distrito_id:
            val_district = move.dgi_partner_distrito_id.name
        else:
            val_district = partner.city or "" # Fallback if no district field

        if move.dgi_partner_provincia_id:
            val_state = move.dgi_partner_provincia_id.name
